...
BX.GanttChart = function(domNode, currentDatetime, settings)
{
	...
	this.currentDatetime = GanttDate.isDate(currentDatetime) ? BX.GanttChart.convertDateToUTC(currentDatetime) : BX.GanttChart.convertDateToUTC(new Date());
	...
	this.calendar = new GanttCalendar(this.settings);
...
GanttProject.prototype.createItem = function()
{
	...
	this.layout.item = BX.create("div", {
		...
		children : [
			...
			BX.create("span", { props : { className: "task-gantt-item-name" }, text : this.name}),
...
var GanttTask = function(chart, id, name, status, dateCreated)
{
	...
	this.dateCreated = GanttDate.isDate(dateCreated) ? BX.GanttChart.convertDateToUTC(dateCreated) : this.chart.currentDatetime;
...
GanttTask.prototype.setTaskFromJSON = function(json)
{
	...
	if (GanttDate.isDate(json.dateEnd) && json.dateEnd > this.dateStart)
	{
	...
	if (!this.isRealDateEnd)
	{
		if (this.status === "completed")
		{
			if (this.dateCompleted == null ||
				...
				GanttDate.getDurationInHours(this.dateStart, this.dateCompleted) < this.chart.endlessBarDefaultDuration
			)
			{
				this.dateEnd = GanttDate.add(this.dateStart, GanttDate.Unit.Hour, this.chart.endlessBarDefaultDuration);
			}
			else
			{
		...
		{
			this.dateEnd = GanttDate.ceilDate(this.chart.currentDatetime, GanttDate.Unit.Day, 1);
			if (this.dateEnd <= this.dateStart ||
				GanttDate.getDurationInHours(this.dateStart, this.dateEnd) < this.chart.endlessBarDefaultDuration)
			{
				this.dateEnd = GanttDate.add(this.dateStart, GanttDate.Unit.Hour, this.chart.endlessBarDefaultDuration);
			}
		}
...
GanttTask.prototype.setRealDates = function()
{
	...
	if (this.matchWorkTime)
	{
		...
		var msInHour = GanttDate.getUnitRatio(GanttDate.Unit.Milli, GanttDate.Unit.Hour);
...
GanttTask.prototype.schedule = function(constraint, transaction)
{
	...
	if (constraint && this.transaction !== transaction)
	{
		...
		var duration = this.calculateDuration();
		var endDate = GanttDate.add(startDate, GanttDate.Unit.Milli, duration);
...
{
	//Scale options
	this.topUnit = config.topUnit || this.topUnit || GanttDate.Unit.Month;
	this.topIncrement = config.topIncrement || this.topIncrement || 1;
	this.topDateFormat = config.topDateFormat || this.topDateFormat || "F Y";

	this.bottomUnit = config.bottomUnit || this.bottomUnit || GanttDate.Unit.Day;
	...
	this.snapUnit = config.snapUnit || this.snapUnit || GanttDate.Unit.Hour;
	...
	//Start-End
	var currentDateMin = GanttDate.floorDate(this.chart.currentDatetime, this.topUnit, this.firstWeekDay);
	var currentDateMax = GanttDate.ceilDate(this.chart.currentDatetime, this.topUnit, this.topIncrement, this.firstWeekDay);

	var snapUnitsInViewport = Math.ceil(this.chart.chartContainer.width / this.getUnitInPixels(this.snapUnit));
	var snapUnitsInTopHeader = GanttDate.getDurationInUnit(currentDateMin, currentDateMax, this.snapUnit);

	var increment = Math.ceil(snapUnitsInViewport / snapUnitsInTopHeader);
	this.startDate = GanttDate.add(currentDateMin, this.topUnit, -increment);
	this.endDate = GanttDate.add(currentDateMax, this.topUnit, increment);
};
...
{
	var duration = GanttDate.getDurationInUnit(this.startDate, this.endDate, this.snapUnit);
...
Timeline.prototype.autoExpandTimeline = function(dates)
{
	...
	for (var i = 0; i < dates.length; i++)
	{
		...
		var currentDateMin = GanttDate.floorDate(date, this.topUnit, this.firstWeekDay);
		var currentDateMax = GanttDate.ceilDate(date, this.topUnit, this.topIncrement, this.firstWeekDay);
		var snapUnitsInTopHeader = GanttDate.getDurationInUnit(currentDateMin, currentDateMax, this.snapUnit);
		var increment = Math.ceil(snapUnitsInViewport / snapUnitsInTopHeader);

		var duration = GanttDate.getDurationInUnit(this.startDate, date, this.snapUnit);
		if (duration <= snapUnitsInViewport)
		{
			var newStartDate = GanttDate.add(currentDateMin, this.topUnit, -increment);
			this.expandTimelineLeft(newStartDate);
			continue;
		}

		duration = GanttDate.getDurationInUnit(date, this.endDate, this.snapUnit);
		if (duration <= snapUnitsInViewport)
		{
			var newEndDate = GanttDate.add(currentDateMax, this.topUnit, increment);
...
Timeline.prototype.expandTimelineLeft = function(date)
{
	...
	var duration = GanttDate.getDurationInUnit(this.startDate, oldDate, this.snapUnit);
	var unitSize = this.getUnitInPixels(this.snapUnit);
...
Timeline.prototype.createHeader = function(start, end, position, unit, increment)
{
	...
	{
		var nextDate = GanttDate.min(GanttDate.getNext(startDate, unit, increment, this.firstWeekDay), endDate);
...
{
	var duration = GanttDate.getDurationInUnit(start, end, this.snapUnit);
	var unitSize = this.getUnitInPixels(this.snapUnit);
...
Timeline.prototype.renderBottomHeader = function(start, end)
{
	...
	if (this.bottomUnit !== GanttDate.Unit.Month &&
		this.bottomUnit !== GanttDate.Unit.Quarter &&
		this.bottomUnit !== GanttDate.Unit.Year)
	{
...
{
	var date = GanttDate.add(this.startDate, this.snapUnit, Math.floor(pixels / this.getUnitInPixels(this.snapUnit)));
...
{
	return GanttDate.getUnitRatio(this.bottomUnit, unit) * this.columnWidth / this.bottomIncrement;
};

Timeline.prototype.snapDate = function(date)
{
	var newDate = new Date(date.getTime());
	if (this.snapUnit === GanttDate.Unit.Day)
	{
		var days = GanttDate.getDurationInDays(this.startDate, newDate);
		var snappedDays = Math.round(days / this.snapIncrement) * this.snapIncrement;
		newDate = GanttDate.add(this.startDate, GanttDate.Unit.Day, snappedDays);
	}
	else if (this.snapUnit === GanttDate.Unit.Hour)
	{
		var hours = GanttDate.getDurationInHours(this.startDate, newDate);
		var snappedHours = Math.round(hours / this.snapIncrement) * this.snapIncrement;
		newDate = GanttDate.add(this.startDate, GanttDate.Unit.Minute, snappedHours * 60);
	}
	else if (this.snapUnit === GanttDate.Unit.Minute)
	{
		var minutes = GanttDate.getDurationInMinutes(this.startDate, newDate);
		var snappedMinutes = Math.round(minutes / this.snapIncrement) * this.snapIncrement;
		newDate = GanttDate.add(this.startDate, GanttDate.Unit.Second, snappedMinutes * 60);
	}
	else if (this.snapUnit === GanttDate.Unit.Week)
	{
		newDate.setUTCHours(0, 0, 0, 0);
		...
		newDate = GanttDate.add(newDate, GanttDate.Unit.Day, daysToSnap);
	}
	else if (this.snapUnit === GanttDate.Unit.Month)
	{
		var months = GanttDate.getDurationInMonths(this.startDate, newDate) + (newDate.getUTCDate() / GanttDate.getDaysInMonth(newDate));
		var snappedMonth = Math.round(months / this.snapIncrement) * this.snapIncrement;
		newDate = GanttDate.add(this.startDate, GanttDate.Unit.Month, snappedMonth);
	}
	else if (this.snapUnit === GanttDate.Unit.Second)
	{
		var seconds = GanttDate.getDurationInSeconds(this.startDate, newDate);
		var snappedSeconds = Math.round(seconds / this.snapIncrement) * this.snapIncrement;
		newDate = GanttDate.add(this.startDate, GanttDate.Unit.Milli, snappedSeconds * 1000);
	}
	else if (this.snapUnit === GanttDate.Unit.Milli)
	{
		var millis = GanttDate.getDurationInMilliseconds(this.startDate, newDate);
		var snappedMilli = Math.round(millis / this.snapIncrement) * this.snapIncrement;
		newDate = GanttDate.add(this.startDate, GanttDate.Unit.Milli, snappedMilli);
	}
	else if (this.snapUnit === GanttDate.Unit.Year)
	{
		var years = GanttDate.getDurationInYears(this.startDate, newDate);
		var snappedYears = Math.round(years / this.snapIncrement) * this.snapIncrement;
		newDate = GanttDate.add(this.startDate, GanttDate.Unit.Year, snappedYears);
	}
	else if (this.snapUnit === GanttDate.Unit.Quarter)
	{
		newDate.setUTCHours(0, 0, 0, 0);
		newDate.setDate(1);
		newDate = GanttDate.add(newDate, GanttDate.Unit.Month, 3 - (newDate.getUTCMonth() % 3));
	}
...
{
	if (!GanttDate.isDate(date) || date < this.startDate || date > this.endDate)
	{
		return;
	}
...
Timeline.prototype.autoScroll = function()
{
	...
	var currentDateInPixels = this.getPixelsFromDate(GanttDate.floorDate(this.chart.currentDatetime, this.snapUnit, this.firstWeekDay));
...
TimelineZoom.prototype.presets = {
	...
	{
		columnWidth: 200,
		topUnit: GanttDate.Unit.Year,
		topIncrement: 1,
		topDateFormat: "Y",
		bottomUnit: GanttDate.Unit.Quarter,
		bottomIncrement: 1,
		bottomDateFormat: "f",
		snapUnit: GanttDate.Unit.Day,
		snapIncrement: 1,
		firstWeekDay: 1
	},
	...
	yearmonth:
	{
		...
		topIncrement: 1,
		topDateFormat: "Y",
		bottomUnit: GanttDate.Unit.Month,
		bottomIncrement: 1,
		bottomDateFormat: "f",
		snapUnit: GanttDate.Unit.Day,
		snapIncrement: 1,
	...
	{
		columnWidth: 24,
		topUnit: GanttDate.Unit.Month,
		topIncrement: 1,
		topDateFormat: "f Y",
		bottomUnit: GanttDate.Unit.Day,
		bottomIncrement: 1,
		bottomDateFormat: "j",
		snapUnit: GanttDate.Unit.Hour,
		snapIncrement: 1
	},
	...
	dayhour: {
		columnWidth: 60,
		topUnit: GanttDate.Unit.Day,
		topIncrement: 1,
		topDateFormat: "j F",
		bottomUnit: GanttDate.Unit.Hour,
		bottomIncrement: 6,
		bottomDateFormat: "G:i",
		snapUnit: GanttDate.Unit.Minute,
		snapIncrement: 15
	},

	hourminute: {
		columnWidth: 50,
		topUnit: GanttDate.Unit.Hour,
		topIncrement: 1,
		topDateFormat: "j F H:i",
		bottomUnit: GanttDate.Unit.Minute,
		bottomIncrement: 15,
...
function GanttCalendar(settings)
{
	//Defaults
...
GanttCalendar.prototype.getClosestWorkTime = function(date, isForward)
{
...
GanttCalendar.prototype.calculateDuration = function(startDate, endDate)
{
	var duration = 0;
	if (startDate < endDate)
	{
...
GanttCalendar.prototype.calculateStartDate = function(endDate, duration)
{
	var newDate = null;
...
GanttCalendar.prototype.calculateEndDate = function(startDate, duration)
{
	var newDate = null;
...
GanttCalendar.prototype.processEachDay = function(startDate, endDate, isForward, callback)
{
	...
	while (endless || (isForward ? currentDate < endDate : currentDate > startDate))
	{
		...
		currentDate = GanttDate.add(GanttDate.floorDate(currentDate, GanttDate.Unit.Day), GanttDate.Unit.Day, isForward ? 1 : -1);
	}
};

GanttCalendar.prototype.getWorkHours = function(date)
{
	var hours = [];
...
GanttCalendar.prototype.isWorkTime = function(date)
{
	if (this.isWeekend(date) || this.isHoliday(date))
	{
		return false;
...
GanttCalendar.prototype.isHoliday = function(date)
{
	var month = date.getUTCMonth();
	var day = date.getUTCDate();
...
GanttCalendar.prototype.isWeekend = function(date)
{
	var day = date.getUTCDay();
	return this.weekends[day] ? true : false;
};

GanttCalendar.prototype.setWeekends = function(weekends)
{
...
GanttCalendar.prototype.setHolidays = function(holidays)
{
...
GanttCalendar.prototype.setWorkTime = function(worktime)
{
...
GanttCalendar.prototype.isWorkTimeCorrect = function(times)
{
	if (!times.length)
	{
		return false;
...
GanttDependency.prototype.getMinDate = function()
{
	...
	else if (this.type === GanttDependency.Type.StartToEnd)
	{
		...
		{
			startDate = GanttDate.add(this.fromTask.dateStart, GanttDate.Unit.Milli, -duration);
		}
	...
	else if (this.type === GanttDependency.Type.EndToEnd)
	{
		...
		{
			startDate = GanttDate.add(this.fromTask.dateEnd, GanttDate.Unit.Milli, -duration);
		}
	}
	else
	{
	...
	{
		return GanttDate.add(startDate, GanttDate.Unit.Milli, this.lag);
	}
};
